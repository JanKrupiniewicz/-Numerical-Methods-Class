function [M,N,P,R,x_coarse,y_coarse,F_coarse,x_fine,y_fine,F_fine] = zadanie4()
    ...
    % Dobór liczby węzłów interpolacji w kierunku osi X i Y
    M = randi([4,8]); % liczba węzłów interpolacji wzdłuż osi X (stopień wielomianu zmiennej x: M-1)
    N = randi([4,8]); % liczba węzłów interpolacji wzdłuż osi Y (stopień wielomianu zmiennej y: N-1)
    ...
    F_coarse = sin(2*pi*X_coarse) .* cos(2*pi*Y_coarse); % przykładowa funkcja interpolowana
    ...
    % Obliczanie wartości wielomianu interpolującego na gęstszej siatce
    for i = 0:(M-1)
        for j = 0:(N-1)
